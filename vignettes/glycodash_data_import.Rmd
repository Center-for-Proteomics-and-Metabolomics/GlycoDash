...
data <- dplyr::left_join(data, plate_design, by = "plate_well") %>% 
  distinct()
...
metadata_recovida_file <- system.file("inst",
                                      "extdata",
                                      "recovida_metadata.rds",
                                      package = "glycodash")
metadata_recovida <- load_and_assign(metadata_recovida_file)

metadata_cleaned <- metadata_cleaned %>% 
  dplyr::rename(sample_id = `ID on Sample tube`)

a <- dplyr::full_join(metadata_recovida, metadata_cleaned) %>% 
  distinct()

test <- dplyr::left_join(data, a, by = "sample_id", suffix = c("_data", "_meta")) %>% 
  distinct()

a <- test %>% dplyr::select(duplicate, duplicate_data, duplicate_meta)
```
