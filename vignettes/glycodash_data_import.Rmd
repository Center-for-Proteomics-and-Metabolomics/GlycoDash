...
# make this into a read_plate_design() function?

plate_design <- readxl::read_excel(system.file("extdata",
                                               "Plate_design.xlsx",
                                               package = "glycodash"))
...
plate_design <- plate_design %>%
  ...
  # tidyr::extract(col = sample_id, into = c("sample_type"), regex = "(.+_|[[:alpha:]]+)", 
  #        remove = FALSE) %>% 
  # dplyr::mutate(sample_type = stringr::str_remove(sample_type, "_"))
...
# make a read metadata function, choose date columns?
...
# Need to make user select the column that contains the sample_ids
# What to do about reading in dates?
