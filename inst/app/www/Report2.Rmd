---
title: "GlycoDash v1.7.1 data processing report"
output: 
  html_document:
    toc: TRUE
    toc_float: TRUE
...
if (!is.null(params$quantitation_protein_tab_contents)) {
  ...
  purrr::iwalk(params$quantitation_protein_tab_contents, function(results, protein) {
    ...
    # ChatGPT workaround for non-interactive correlation plot
    if (!is.null(contents$correlation_plots)) {
      cat("\nPeptide correlation plots:\n\n")
      tmpfile <- tempfile(fileext = ".png")
      ggplot2::ggsave(tmpfile, plot = contents$correlation_plots, width = 8, height = 6, dpi = 200)
      encoded <- base64enc::dataURI(file = tmpfile, mime = "image/png")
      cat(sprintf('<img src="%s" width="600"/>\n', encoded))
      cat("\n Correlation method: ", contents$correlation_method)
      cat("\n \n")
    }
  })
}
...
if (!is.null(params$site_occupancy_quality_plot)) {
  ...
  cat("\n \n Peptide ions excluded from site occupancy calculations: ", paste0(
    params$site_occupancy_excluded_peptides, collaps = ", "
  ))
}
