...
merged_metadata <- purrr::reduce(metadata_list,
                                 dplyr::full_join,
                                 by = "sample_id")
...
Currently derived traits can only be calculated for IgG data. The user can select which of the 4 derived traits should be calculated. The derived traits are calculated per cluster.

```{r}
path_to_traits_formulas <- system.file("extdata",
                                       "traits_formulas.xlsx",
                                       package = "glycodash")
...
test_expr <- create_expr_ls("my_trait = H3N4F1 + H4N5")


custom_traits <- calculate_custom_traits(normalized_data, custom_traits_formulas)
...
# The example data has only IgG1 analytes. 
# The IgGII custom trait column now only has NA values.
# Will have to check if it works with other clusters than IgG1.

# Next:
# Show formulas used in dashboard
# Combine custom traits with calculated default traits
```
