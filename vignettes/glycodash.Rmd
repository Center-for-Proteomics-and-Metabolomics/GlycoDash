...
n_colors <- length(unique(data_curated$reason_for_failure)) - 1
...
reasons <- unique(data_curated$reason_for_failure)
...
reason_labels <- paste("No,", reasons) %>% 
  ...
  rlang::set_names(., nm = unique(
    ...
    # empty:
    replace(data_curated$reason_for_failure,
            data_curated$reason_for_failure == "",
            "none")
  ))
reason_labels[reason_labels == "No, "] <- "Yes"

my_data <- data_curated %>% 
  ...
  dplyr::mutate(
    ...
    # Replace the empty values "" in reason_for_failure with "none"  to matche
...
for_upset <- to_plot %>% 
  ...
  dplyr::mutate(specific_total = group,
                .keep = "unused") %>% 
  dplyr::group_split(analyte)

purrr::map(for_upset,
           ~ my_upset(.x) %>% 
             ggpubr::ggpar(., title = unique(.x$analyte)))
...
derived_traits <- calculate_derived_traits(normalized_data, "Fucosylation") %>% 
        tidyr::pivot_wider(names_from = cluster,
                           values_from = "Fucosylation",
