  ...
  tagList(
    fluidPage(
      ...
      fluidRow(
        column(
          width = 6,
          mod_read_data_ui(ns("read_data_ui_1")),
...
mod_data_import_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    data_input <- mod_read_data_server("read_data_ui_1")
    ...
    data_incl_sample_types <- mod_add_sample_types_server(
      ...
      data = data_incl_sample_ids$data
    )
    
    data_incl_clusters <- mod_clusters_server(
      "clusters_ui_1",
      data = data_incl_sample_types$data
    )
    
    data_incl_metadata <- mod_add_metadata_server(
      "add_metadata_ui_1",
      data = data_incl_clusters$data
    )
    ...
    # When data has been read in, the converted data is shown
    # in the data table
    ...
    output$download_glycosites <- downloadHandler(
      ...
      content = function(file) {
        data_to_download <- data_input$glycosites_table()
