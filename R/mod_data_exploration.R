      ...
      fluidRow(
        ...
        div(
          ...
          shinydashboard::box(
            ...
            tabsetPanel(
              id = ns("tabs"),
              tabPanel(title = "Figure 1", uiOutput(ns("first_tab")))
            )
          )
        )
      )
...
#'
#' @noRd 
mod_data_exploration_server <- function(
    id, 
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      req(my_data())
      tab_results$tab1 <- mod_tab_data_exploration_server(
        "tab1", my_data = my_data, trigger = reactive(r$trigger)
      )
    })
    ...
    observe({
      ...
      tab_results[[
        stringr::str_replace(input$remove_tab, "Figure ", "tab")
      ]] <- NULL
