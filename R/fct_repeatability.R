...
find_choices_for_repeatability_menu <- function(normalized_data) {
  ...
  if ("group" %in% colnames(menu_df)) {
    choices <- purrr::pmap(
      menu_df, function(group, sample_id) {
...
calculate_repeatability_stats <- function(
    data,
    standard_sample_id,
    standard_group  
  ) {
  ...
  if (all(purrr::map_lgl(repeatability, rlang::is_empty))) {
    rlang::abort(
      class = "no_samples",
      message = paste(
        "There are no samples of this sample_id",
  ...
  repeatability <- repeatability %>% 
    ...
    dplyr::reframe(
      average_abundance = mean(relative_abundance),
...
visualize_repeatability <- function(
    repeatability_data,
    selected_group = NULL,
    selected_sample_id  
  ) {
  ...
  if (nrow(repeatability_data) == 0) {
    rlang::abort(
      ...
      message = paste0(
        "For all",
        ifelse(!is.null(selected_group), paste0(" ", selected_group), ""),
    ...
    ggplot2::geom_col(
      ggplot2::aes(
        ...
        text = paste0(
          ...
          format(round(average_abundance, digits = 2), nsmall = 2),
    ...
    ggplot2::geom_point(
      ggplot2::aes(
        ...
        text = paste0(
          ...
          plate,
          "\nRSD: ",
          format(round(RSD, digits = 2), nsmall = 2),
          # signif(RSD, 3),
          "%"
        )
      ),
  ...
  ggplot2::scale_y_continuous(
    ...
    ggplot2::scale_fill_manual(values = my_palette, name = "Plate") +
...
#'
#' @return A ggplot object.
visualize_repeatability_mean_bars <- function(
    data,
    selected_sample_id,
    selected_group = NULL  
  ) {
  ...
  mean_barplot <- ggplot2::ggplot(to_plot) +
    ...
    ggplot2::geom_errorbar(
      ggplot2::aes(
        ymin = mean_rel_ab - sd_rel_ab,
        ymax = mean_rel_ab + sd_rel_ab,
        x = analyte,
        text = paste0(
          "Standard deviation: ",
          format(round(sd_rel_ab, digits = 2), nsmall = 2),
          # signif(sd_rel_ab, 3),
          "%"
        )
      ),
